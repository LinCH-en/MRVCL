...
cat_train_file=''
cat_test_file=''
static_aug_train_file=''
static_aug_test_file=''
dynamic_aug_train_file=''
dynamic_aug_test_file=''
poi_time_file=''
poi_cat_file=''
...
def train(loc_emb,g_dropout,alpha,beta,gamma,s_dim,r_dim,hidden_units):
    model=PreSelfMove(emb_dim=loc_emb, out_size=loc_emb, num_heads=2, dropout=g_dropout, node_num=TOTAL_POI,
    ...
    trainT = trainT + trainT[-(batch_size - train_size):]
    ...
    testT = testT + testT[-(batch_size - test_size):]
    ...
    while (Flag):
        ...
        if (epoch+1)%2==0:
            model.eval()
            torch.save(model.state_dict(), '..\processed\pretrains/NYC_'+str(alpha)+'_'+str(beta)+'_'+str(gamma)+'_'+str(loc_emb)+'_'+str(g_dropout)+'_'
                       +str(s_dim)+'_'+str(r_dim)+'_'+str(hidden_units) + '.pth')



train(loc_emb=,g_dropout=,alpha=,beta=,gamma=,s_dim=,r_dim=,hidden_units=)
