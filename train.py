...
def train(loc_emb,g_dropout,alpha,beta,gamma,s_dim,r_dim,hidden_units):
    ...
    pretrained_dict = torch.load(
        r'..\processed\pretrains\TKY_1_1_0.1_256_0_256_32_300.pth')
    model_dict = model.state_dict()
    ...
    fw_res = open('Result_NYC_ooo.txt', 'a')
    fw_res.flush()
...
def output(index, ACC_1, ACC_5, ACC_10,auc,map):
    fw_res = open('../Result_NYC_hhh.txt', 'a')
    fw_res.flush()
    fw_res.write('epoch' + str(index) + '\tACC@1\t' + str(ACC_1) + '\tACC@5\t' + str(ACC_5) + '\tACC@10\t' + str(ACC_10) +
                 '\t' + str(auc)+'\t'+str(map)+ '\n')
    fw_res.close()
train(loc_emb=256,g_dropout=0,alpha=1,beta=1,gamma=0.1,s_dim=256,r_dim=32,hidden_units=256)